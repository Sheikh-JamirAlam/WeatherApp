...
function App() {
  ...
  return (
    <div
      className={`h-full w-full flex bg-center`}
      style={{
        backgroundImage: `${isClickedSearch ? `url("./assets/${data.weather[0].main.toLowerCase()}.jpg")` : 'url("./assets/clear.jpg")'}`
      }}
    >
      <div className="flex flex-col place-content-center mx-auto font-josefin font-normal text-lg text-gray-200">
        ...
        >
          <span className={`material-symbols-outlined text-3xl place-self-center ml-3 cursor-pointer ${!isSearchDisabled ? "visible relative" : "invisible absolute"}`}>home_pin</span>
          <input
            type="text"
            name="searchtext"
